...
class ContentEditor:
    def __init__(self, app):
        ...
        self.layout = [
            ...
            [
                sg.Button('Save Custom Values', key='-SAVE-CUSTOM-VALUES-', disabled=True),
                sg.Button('Close')
            ]
        ]
        ...
        self.window['-SAVE-CUSTOM-VALUES-'].bind('<ButtonRelease-1>', self.handle_save_custom_values)
    ...
    def handle_save_custom_values(self, event):
        data = self.table.get()
        df_custom = pd.DataFrame(data, columns=['Row', 'Element Name', 'Default Value', 'Custom Value'])
