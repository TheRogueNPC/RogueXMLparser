...
class SchemaWindow:
    def __init__(self, df_xml, tree):
        ...
        layout = [
            [sg.Column([[sg.Multiline('', size=(50, 20), key='-SCHEMA-', disabled=True)]], size=(None, None))],
        ...
        self.window['-SCHEMA-'].Widget.config(wrap='none')  # Disable line wrapping for the text field
    ...
    def generate_xml_with_schema_comments(self):
        ...
        for _, row in self.df_xml.iterrows():
            ...
            for elem in root.iter(tag):
                ...
                if elem.text is not None and elem.text.strip(): # Check if the element has text content
    ...
    def export_xml(self):
        ...
        save_path = sg.popup_get_file('Save XML File', save_as=True, file_types=(('XML Files', '*.xml'),))
        if save_path:
