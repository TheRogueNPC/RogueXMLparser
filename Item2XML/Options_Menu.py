...
class OptionsMenu:
    ...
    def create_layout(self):
        ...
        layout = [
            [
                sg.Text("Window Color:"),
                sg.Input(key="-WINDOW_COLOR-", background_color="white"),
                sg.ColorChooserButton("Choose", key="-CHOOSE_WINDOW_COLOR-"),
            ],
            [
                sg.Text("Button Color:"),
                sg.Input(key="-BUTTON_COLOR-", background_color="white"),
                sg.ColorChooserButton("Choose", key="-CHOOSE_BUTTON_COLOR-"),
            ],
            [
                sg.Text("Font Color:"),
                sg.Input(key="-FONT_COLOR-", background_color="white"),
                sg.ColorChooserButton("Choose", key="-CHOOSE_FONT_COLOR-"),
            ],
            [
                sg.Text("Default Window Size:"),
                sg.Input(
                    key="-WINDOW_SIZE-",
                    default_text=self.settings.get("-WINDOW_SIZE-", ""),
                ),
            ],
            [
                sg.Checkbox(
                    ...
                    key="-LOCK_RESIZE-",
                    default=self.settings.get("-LOCK_RESIZE-", "false").lower()
                    == "true",
                )
            ],
    ...
    def run(self):
        ...
        while True:
            ...
            if event == sg.WINDOW_CLOSED or event == "Cancel":
                ...
                self.settings["-LOCK_RESIZE-"] = (
                    "true" if values["-LOCK_RESIZE-"] else "false"
                )
                ...
                if new_color is not None:
                    ...
                    self.window["-WINDOW_COLOR_LABEL-"].update(
                ...
                if new_color is not None:
                    self.window[color_key].update(new_color)
                    self.window[color_key].set_background_color(new_color)
                    self.window["-BUTTON_COLOR_LABEL-"].update(
                        background_color=new_color
                    )
